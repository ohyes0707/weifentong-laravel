...
<html>
	...
	<body>
		 ...
		 @include('Agent.common.left')
			...
			<div class="main-right">
				...
				<form action="" method="post" id="f_add" class="form">
					...
					<div class="form-list">
						<label for="gh-name">公众号：</label><input onblur="checkWxName();" value="{{ $report['wx_name'] }}" name="wx_name" type="text" id="gh-name" class="text-input"/>
       ...
       <div class="form-list">
           <label for="company">公司名称：</label><input onblur="checkCompany();" value="{{ $report['company'] }}" name="company" type="text" id="company" class="text-input"/>
       ...
       <div class="form-list">
           <label for="linkman">联系人：</label><input onblur="checkContacts();" value="{{ $report['contacts']}}" name="contacts" type="text" id="linkman" class="text-input"/>
       ...
       <div class="form-list">
           <label for="linkway">联系方式：</label><input onblur="checkTelphone();" value="{{ $report['telphone'] }}" name="telphone" type="text" id="linkway" class="text-input"/>
            ...
            <select name="agent" style="width: 300px;height: 34px;border: 1px solid #ccc;border-radius: 4px;padding: 0 10px;float: left;">
                    @if(count($data)>0)
                        ...
                        <option value="{{ $agentList['id'] }}" 
                                @if(isset($report['agent_id'])&&$agentList['id'] == $report['agent_id'])
                                selected
                                @endif
                                >{{ $agentList['nick_name'] }}</option>
                        @endforeach
                    @endif
                </select>   
            <p class="tip exist" id="telphone_error" style="display: none;"></p>
        ...
        <div class="form-list">
            <label>绑定方式：</label>
            <label class="label-radio l_radio onradio">{{--<input onclick="alert('本期只开放门店类型公众号报备')" type="button">--}}<input @if(isset($report['type']) && $report['type']==1) checked="checked" @endif onclick="checkWxType()" type="radio" name="type" value="1">公众号授权</label>
            <label class="label-radio offradio"><input @if(isset($report['type']) && $report['type']==2)  checked="checked" @endif onclick="checkWxType()" type="radio" name="type" value="2">门店添加</label>
