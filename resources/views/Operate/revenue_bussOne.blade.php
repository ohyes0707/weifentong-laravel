				...
				<table @if(isset($list) && !empty($list))style="display: none;"@endif class="table-bordered table-responsive table-striped detail init-table colspan-7">
					...
					<tr class="f_channel off_channel" channelId="{{$v['id']}}">
						...
						<td>
							...
							<a href="revenueDetail_buss?bid={{$v['id']}}&bname={{$v['nick_name']}}" class="btn btn-blue">查看公众号</a>
...
//		        startDate:new Date()
			...
			$(".reset").click(function(){
				...
				window.location.href='revenueCount_bussOne?buss='+buss;
			})
			$(".find").click(function(){
				$("input[name=excel]").val(0);
				$("#f-report").attr('action','revenueCount_bussOne');
			...
			$(".excel").click(function(){
				...
				var action = $("#f-report").attr('action');
				$("#f-report").submit();
			})
		})
	</script>
