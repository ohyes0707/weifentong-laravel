			...
			var str='<tr><th>渠道名称</th><th>获取公众号</th><th>成功获取公众号</th><th>填充率</th><th>微信认证</th><th>认证率</th><th>成功关注</th><th>关注率</th></tr>';
				...
				success: function(data){
                                   ...
                                   $.each(arr,function(i,v){ 
                                        ...
                                        str+='<tr><td>'+v.pbusername+'(汇总)</td><td>'+v.sumgetwx+'</td><td>'+v.getwx+'</td><td>'+fillrate+'%</td><td>'+v.complet+'</td><td>'+confirmrate+'%</td><td>'+v.follow+'</td><td>'+followate+'%</td></tr>'
                                            $.each(v.list,function(ii,vv){
                                                var vfillrate = division(vv.getwx,vv.sumgetwx);
                                                var vconfirmrate = division(vv.complet,vv.getwx);
                                                var vfollowate = division(vv.follow,vv.complet);
                                                str+='<tr><td>'+vv.username+'</td><td>'+vv.sumgetwx+'</td><td>'+vv.getwx+'</td><td>'+vfillrate+'%</td><td>'+vv.complet+'</td><td>'+vconfirmrate+'%</td><td>'+vv.follow+'</td><td>'+vfollowate+'%</td></tr>'
                                           });
                                       });
...
//					str+='<tr><td>'+data.data.sumgetwx+'</td><td>'+data.data.sumgetwx+'</td><td>'+data.data.sumgetwx+'</td><td>'+data.data.sumgetwx+'</td><td>'+data.data.sumgetwx+'</td><td>'+data.data.sumgetwx+'</td><td>'+data.data.sumgetwx+'</td><td>'+data.data.sumgetwx+'</td></tr>'
		...
		function getData3(){
			var select='<option value="">全部</option>';
                ...
                function getData4(){
			var select='<option value="">全部</option>';
                        var select2='<option value="">全部</option>';
                        var pb={{$pbid}};
                        $.ajax({
				type:"get",
				url:"http://{{Config::get('config.API_URL')}}data/getSumDesc/v1.0",
				success: function(data){
                                   ...
                                   $.each(arr,function(i,v){ 
                                       if(pb==i){
                                          select+='<option value="'+i+'" selected>'+v.pbusername+'</option>';
                                            $.each(v.list,function(ii,vv){
                                                   select2+='<option value="'+vv.bussid+'">'+vv.username+'</option>'; 
