			...
			<div class="main-right">
				...
				<div class="main-top clearfix">
					<div class="oprate-btns">
						<a href="{{URL::asset('index.php/operate/addRole?')}}" class="btn btn-blue">新增</a>
				...
				<table class="table-bordered table-responsive detail d-table">
					...
					@foreach($data as $key=>$value)
						<tr>
							<td><input type="checkbox" value="{{$value['title']}}" name="checkbox"/></td>
							<td>{{$value['title']}}</td>
							<td>
								<a href="{{URL::asset('index.php/operate/editRole?id='.$value['id'])}}" class="btn btn-blue">编辑</a>
								<button id="{{$value['id']}}" name="{{$value['title']}}" class="btn btn-red btn-delete">删除</button>
							</td>
						</tr>
					@endforeach
				</table>
	...
	<script type="text/javascript">
        $(function(){
			...
			$('.btn-sure').click(function(){
				...
				$.getJSON('{{URL::asset('index.php/operate/roleList') }}', {"type": 'delete', "name": name},
						function (data) {
							if (data.data > 0) {
