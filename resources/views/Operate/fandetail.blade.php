...
<!--					    <select name="parentid" class="form-control">
					    	...
					    	<option value="{{$datalist['parent_id']}}">
                                                        @if($datalist['parent_id']==$termarray['wx_id'])
                                                        selected
                                                        @endif
					    	...
					    	@endforeach
                                            ...
                                            <input type="text" id="inputgzh" hidden name="parentid" class="form-control" style="display: none">
                                            <input type="text" id="inputv" onkeyup="checkFunction()" name="ParentName"  class="form-control" autocomplete="off" value="">
                                                    <ul id="ul_hide">
                                                        @if(count($bussname)>0)
                                                            @foreach ($bussname as $key => $datalist)
					...
					<input type="button" value="查询" class="find"/>	
                                        ...
                                        <input type="text" hidden="" value="{{ $termarray['wx_id'] }}" name="wxid"/>
				...
				<table class="table-bordered table-responsive table-striped detail init-table colspan-4">
					<tr>
						...
						<th style="width: 20%">成功关注</th>
						<th style="width: 20%">取关量</th>
						<th style="width: 20%">取关率</th>
				...
				@foreach ($paginator as $key => $datalist)
					<tr class="f_channel off_channel" channelId="{{ $key }}">
						...
						<td>{{$datalist['unfollowrate']}}%</td>
                                                <td><a href="getFanBussOneReport?parentid={{$datalist['parent_id']}}&wxid={{ $termarray['wx_id'] }}&usertype={{$termarray['usertype']}}&startdate={{$termarray['startdate']}}&enddate={{$termarray['enddate']}}" class="option_detail">查看子渠道</a></td>
					</tr>
					...
					if($sub.is(':hidden')){
                        ...
                        $('.find').click(function(){
                            ...
                            if($('.form-control').val()>0||$('#inputv').val()!=''){
                                var path = "getFanBussOneReport";  
                                $('.form-inline').attr("action", path).submit();
                            }else{
                                var path = "getFanDetail";  
                                $('.form-inline').attr("action", path).submit();
                            }
                        })
                        
                        $('.excel').click(function(){
                            $('.form-inline').attr("action", 'getFanDetailExcel').submit();
                        })
                    $("#datetimeStart").val("{{ $termarray['startdate'] }}");
                    $("#datetimeEnd").val("{{ $termarray['enddate'] }}");
                    
                    $(".reset").click(function () {
			window.location.href = 'getFanDetail?wxid={{$termarray['wx_id']}}';
                    })
		})
	</script>
</html>
