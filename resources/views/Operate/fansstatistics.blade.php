...
<!--					    <select name="wxid" class="form-control">
					    	...
					    	<option value="{{$datalist['wx_id']}}">{{$datalist['wx_name']}}</option>
					    	@endforeach
                                                @endif
					    </select>	-->
                                            <input type="text" id="inputgzh" hidden name="wxid" class="form-control" style="display: none">
                                            <input type="text" id="inputv" onkeyup="checkFunction()" name="wxName"  class="form-control" autocomplete="off">
                                                    <ul id="ul_hide">
                                                        @if(count($wxid)>0)
                                                            @foreach ($wxid as $wx)
				...
				<table class="table-bordered table-responsive table-striped detail init-table colspan-8">
					...
					<tr class="f_channel off_channel" channelId="{{ $key }}">
						...
						<td><a href="getFanDetail?wxid={{$datalist['wx_id']}}&usertype={{$termarray['usertype']}}&startdate={{$termarray['startdate']}}&enddate={{$termarray['enddate']}}" class="option_detail">查看渠道</a></td>
					</tr>
					...
					if($sub.is(':hidden')){
                        ...
                        $('.find').click(function(){
                            ...
                            if($('.form-control').val()>0||$('#inputv').val()!=''){
                                $('.form-inline').attr("action", 'getFansOneReport').submit();
                                //window.location.href="getFansOneReport?id="+$('.form-control').val(); 
                            }else{
                                $('.form-inline').attr("action", 'getFansReport').submit();
                            }
                        })
