			...
			<div class="main-right">
				...
				<div class="main-top clearfix">
					<div class="oprate-btns">
						<a href="/operate/agent/addAgency" class="btn btn-blue">新增</a>
					...
					<form action="" method="post" class="checkform">
						...
						<input type="text" name="agency" @if(isset($agency))value="{{$agency}}" @endif placeholder="查询代理" class="input"/>
					...
					@if(isset($list) && !empty($list))
						@foreach($list as $k=>$v)
							<tr>
								...
								@endif
								<td>
									<a href="agentEdit?uid={{$v['id']}}" class="btn btn-blue">编辑</a>
									<a href="agentForm?uid={{$v['id']}}&uname={{$v['nick_name']}}" class="btn btn-blue">报表</a>
									<a href="subAgent?uid={{$v['id']}}&uname={{$v['nick_name']}}" class="btn btn-blue">子代理</a>
									@if($v['status'] == 1)
		...
		$(function(){
			...
			$('.oprate-btns .btn-use').click(function(){
				...
				$('.btn-sure').click(function(){
					$.post('/operate/user/startAll',{'_token':token,'str':str},function(t){
						$('.myalert').hide()
						window.location.href='/agent/user/agentList?page='+page;
					})
				})
			})
			$('.oprate-btns .btn-forbidden').click(function(){
				...
				$('.btn-sure').click(function(){
					$.post('/operate/user/endAll',{'_token':token,'str':str},function(t){
						$('.myalert').hide()
						window.location.href='/agent/user/agentList?page='+page;
					})
				})
			...
			$('.detail .btn-forbidden').each(function(){
				$(this).click(function(){
					...
					$('.btn-sure').click(function(){
						$.post('/operate/user/status',{'_token':token,'uid':value},function(t){
							$('.myalert').hide();
							window.location.href='/agent/user/agentList?page='+page;
						});
					})
			...
			$('.oprate-btns .btn-delete').click(function(){
				...
				$('.btn-sure').click(function(){
					$.post('/operate/user/delAll',{'_token':token,'str':str},function(t){
						$('.myalert').hide()
						window.location.href='/agent/user/agentList?page='+page;
					})
				})
			...
			$('.detail .btn-delete').each(function(){
				$(this).click(function(){
					...
					$('.btn-sure').click(function(){
						$.post('/operate/user/del',{'_token':token,'uid':value},function(t){
							$('.myalert').hide();
							window.location.href='/agent/user/agentList?page='+page;
						});
					})
