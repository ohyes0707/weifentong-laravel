					...
					@if(isset($data) && !empty($data))
						@foreach($data as $k=>$v)
							<tr class="f_channel off_channel" channelId="{{$v['id']}}">
								...
								{{--<td>{{$v['count']['rest']}}</td>--}}
								<td><a href="revenueDetail_wechat?buss={{$v['id']}}&wid={{$wid}}&wx_name={{$wx_name}}"}}>查看子渠道</a></td>
							</tr>
...
//		        startDate:new Date()
			...
			$(".reset").click(function(){
				...
				window.location.href='revenueDetail_wechatOne?wid='+wid+'&wx_name='+wx_name+'&buss='+buss;
			})
			$(".find").click(function(){
				$("input[name=excel]").val(0);
				$("#f-report").attr('action','revenueDetail_wechatOne');
