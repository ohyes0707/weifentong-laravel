					...
					@if(isset($list) && !empty($list))
						@foreach($list['father'] as $k=>$v)
							<tr class="f_channel off_channel" channelId="{{$v['id']}}汇总">
								...
								<td>{{$v['total']['unfollow']}}</td>
								@if($v['total']['follow'] != 0)
									<td>{{sprintf('%.2f',$v['total']['unfollow']/$v['total']['follow']*100)}}%</td>
								@else
									<td>0.00%</td>
								@endif
							</tr>
							@if(isset($v['count']) && !empty($v['count']))
								@foreach($v['count'] as $kk=>$vv)
									...
									<tr class="sub_channel" fatherId="{{$v['id']}}汇总">
										...
										<td>{{$vv['unfollow']}}</td>
										@if($vv['follow'] != 0)
											<td>{{sprintf('%.2f',$vv['unfollow']/$vv['follow']*100)}}%</td>
										@else
											<td>0.00%</td>
										@endif
									</tr>
	...
	<script type="text/javascript">
		$(function(){
			...
			$(".reset").click(function(){
				window.location.href = 'fansCount';
			})
			$(".find").click(function(){
