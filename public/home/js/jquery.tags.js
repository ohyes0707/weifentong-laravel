    ...
    Tags.prototype = {
        set: function (elems, values) {
            ...
            elems.each(function (key, elem) {
                for (var key in values) {
                    var tag = $('<div class="label-infotag"><span class="label label-info">' + values[key] + '</span><span class="label-close"></span></div>');
                    $(elem).after(tag);
                    //tag.click(click);  no click
                    Tags.value(tag);
                };
            });
        },
        bind: function (elem) {
            ...
            elem.keypress(function (event) {
                ...
                var tag = $('<div class="label-infotag"><span class="label label-info">' + text + '</span></div>');
                self.before(tag).val('');
                tag.click(click);
                Tags.value(elem);
...
$(function () {
    $('head').append($('<style>.form-tags span, .input-tags {padding: .3em .6em .3em;inline-block;font-size: 14px;margin: 4px 4px 0 0;line-height: 26px;font-weight: normal;}.input-tags {width: auto;}</style>'));
