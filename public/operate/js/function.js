...
function formSubmit(){
	...
	var fansdata = $(".fansdata").text();
	var sumall = $(".sum-all").text();
	//if(allsum > allfans){
	...
	if(parseInt(fansdata) == parseInt(sumall)){
		$('.f_order').submit();
	}else{
	//	dataJudge('总涨粉量过小，请重新填写');
		//return false;
	}
}
...
$('.causeMain textarea').bind('keyup',function(){
    ...
    //
	//})
	$('.subset .alladd').on('input propertychange',function(){
		channelSum('.alladd','.sum-all')

	})
	$('.subset .dayadd').on('input propertychange',function(){
		channelSum2('.dayadd','.sum-day')
    ...
    //
	//})
	//function channelSum(obj1,obj2,e){
	//	var e=e || event;
	//	var sum=0,
	//		allsum=0,
	//		code=$(e.target).parents('tr').data('fcode'),
	//		$sub=$('tr[data-fcode="'+code+'"]').find(obj1),
	//		$parent=$('tr[data-code="'+code+'"]').find(obj1);
	//	$sub.each(function(){
	//		if($(this).val()!=''){
	//			sum+=parseFloat($(this).val())
	//		}
	//	})
	//	$('.subset '+obj1).each(function(){
	//		if($(this).val()!=''){
	//			allsum+=parseFloat($(this).val())
	//		}
	//	})
	//	$parent.val(sum);
	//	$(obj2).text(allsum)
	//}
	$('.parentOnly .alladd').on('input propertychange',function(){
		var allsum=0;
		$('.parentOnly .alladd').each(function(){
			if($(this).val()!=''){
				allsum+=parseFloat($(this).val())
			}
		})
		$('.subset .alladd').each(function(){
			if($(this).val()!=''){
				allsum+=parseFloat($(this).val())
			}
		})
	...
	//$('.parentOnly .dayadd').on('input propertychange',function(){
	//	var allsum=0;
	//	$('.parentOnly .dayadd').each(function(){
	//		if($(this).val()!=''){
	//			allsum+=parseFloat($(this).val())
	//		}
	//	})
	//	$('.subset .dayadd').each(function(){
	//		if($(this).val()!=''){
	//			allsum+=parseFloat($(this).val())
	//		}
	//	})
	...
	function channelSum2(obj1,obj2,e){
		var e=e || event;
		var sum=0,
			allsum=0,
			code=$(e.target).parents('tr').data('fcode'),
	...
	//多选框选中和取消
	$(document).delegate('.subset label','click',function(){
		var l=$(this),$input=l.find('input');
		if($input.prop('checked')){
			l.removeClass('offlabel').addClass('onlabel');
