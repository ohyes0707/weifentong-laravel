...
use App\Services\Impl\User\BussServicesImpl;
class UserController extends Controller
{
    ...
    public function login(){
        $userid = session()->get('buss_id');
        if(empty($userid)){
            return view('Business.index');
        }
        return redirect()->intended('business/Fans/fansEarn'); //登录后默认跳转页
    }
    ...
    public function changePwd(){
        if(isset($_POST['pwd'])){
            $parameter = array('userId'=>session()->get('buss_id'));
            $data = HttpRequest::getApiServices('buss','getBussInfo','GET',$parameter);
        ...
        return view('Business.changepwd');
    }
    public function changeOver(){
        if(isset($_POST['new_pwd']) && $_POST['type']=='none'){
            $new_pwd = $_POST['new_pwd'];
            $uid = session()->get('buss_id');
            $BussServicesImpl = new BussServicesImpl();
            $data = $BussServicesImpl->changePwd($uid,$new_pwd);
            if($data){
                return '修改成功';
            }else{
                return '修改失败';
