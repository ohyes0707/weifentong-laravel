...
class WorkOrderController extends Controller
{
    ...
    {
        $parameter['userid']= session()->get('agent_userid');
        ...
        return view('agent.addworkorder', $data);
    }
    ...
    public function postWorkOrder(Request $request)
    {
        ...
        $logary=array(
            'user_id'=>session()->get('agent_userid'),
            'wx_id'=>$wx_array[2],
        ...
        if($model->isValid()){
            if (!$model->add()) {
                return redirect('agent/workorder/getworkorder')->with('msg', '添加失败');
            } else {
                sleep(1);
                return redirect('agent/workorder/getworkorderlist');
            }
        }else {
    ...
    public function getWorkOrderList()
    {
        ...
        $data['userid']=session()->get('agent_userid');
        //根据筛选条件获取信息
        ...
        return view('agent.workorderlist',  $data);
    }
    ...
    public function getEditWorkOrder()
    {
        ...
        return view('agent.editworkorder', $data);
    }
    ...
    public function postEditWorkOrder(Request $request)
    {
        ...
        if($model->isValid()){
            if (!$model->update($request->input('id'),$isorder)) {
                redirect('agent/workorder/getworkorder')->with('msg', '修改失败');
            } else {
                return redirect('agent/workorder/getworkorderlist');
            }
        }else {
